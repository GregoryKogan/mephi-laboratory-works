...
template <class T>
IEnumerator<T>::IEnumerator(Sequence<T> *sequence, int index)
...
template <class T> bool IEnumerator<T>::move_next() {
...
template <class T> T &IEnumerator<T>::current() const {
  return sequence->operator[](index);
}

template <class T> void IEnumerator<T>::reset() { index = 0; }

template <class T> IEnumerator<T> IEnumerator<T>::operator++() {
  move_next();
  return *this;
}
...
template <class T> T &IEnumerator<T>::operator*() const { return current(); }
