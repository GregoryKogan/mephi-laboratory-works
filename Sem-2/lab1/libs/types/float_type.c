...
void float_free_memory(void *instance_ptr) { free(instance_ptr); }
...
type_t *float_type_ctor(error *err) {
  return type_ctor(err, "float", float_get_zero(err), float_get_one(err),
                   float_from_instance, float_free_memory, float_add, float_sub,
                   float_mul, float_random, float_to_string);
}
