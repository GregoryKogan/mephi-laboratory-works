...
public:
  ...
  friend std::ostream &operator<<(std::ostream &os,
                                  const BaseContainer<T> &container) {
    os << container.to_string();
    return os;
  }
};

template <class T> bool BaseContainer<T>::empty() const {
  return sequence->get_length() == 0;
}

template <class T> size_t BaseContainer<T>::size() const {
  return sequence->get_length();
}

template <class T> void BaseContainer<T>::swap(BaseContainer<T> &other) {
  auto tmp = sequence;
...
template <class T>
Sequence<T> *BaseContainer<T>::get_subsequence(int start_index,
                                               int end_index) const {
  return sequence->get_subsequence(start_index, end_index);
}

template <class T> Sequence<T> *BaseContainer<T>::map(T (*func)(T)) const {
...
template <class T> Sequence<T> *BaseContainer<T>::where(bool (*func)(T)) const {
  auto result = new LinkedListSequence<T>;
  for (auto x : *this)
    if (func(x))
      result->append(x);
...
template <class T> std::string BaseContainer<T>::to_string() const {
  if (empty())
    return "[]";
