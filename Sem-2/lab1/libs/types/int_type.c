...
void int_free_memory(void *instance_ptr) { free(instance_ptr); }
...
type_t *int_type_ctor(error *err) {
  return type_ctor(err, "int", int_get_zero(err), int_get_one(err),
                   int_from_instance, int_free_memory, int_add, int_sub,
                   int_mul, int_random, int_to_string);
}
