...
template <class T> class NaryTreeSerializer {
  ...
  Sequence<std::string> *
  parse_serialized_children(const std::string &serialized_children);
  void append_parsed_children(NaryTree<T> *subtree,
...
template <class T> NaryTreeSerializer<T>::~NaryTreeSerializer() { delete tree; }

template <class T> const NaryTree<T> *NaryTreeSerializer<T>::get_tree() const {
  return tree;
}
...
template <class T> void NaryTreeSerializer<T>::serialize() {
...
template <class T> void NaryTreeSerializer<T>::deserialize() {
...
template <class T>
NaryTree<T> *
NaryTreeSerializer<T>::deserialize_child(const std::string &serialized_child) {
  T root_value = read_until_open_bracket(serialized_child);
  auto *result = new NaryTree<T>(root_value, max_children_data);

  Sequence<std::string> *parsed_children =
      parse_serialized_children(parse_content(serialized_child));
...
template <class T>
Sequence<std::string> *NaryTreeSerializer<T>::parse_serialized_children(
    const std::string &serialized_children) {
...
template <class T>
void NaryTreeSerializer<T>::append_parsed_children(
    NaryTree<T> *subtree, const Sequence<std::string> *parsed_children) {
  if (parsed_children->get_length() > max_children_data)
    throw IndexOutOfRangeException((int)parsed_children->get_length(), 0,
                                   max_children_data);
