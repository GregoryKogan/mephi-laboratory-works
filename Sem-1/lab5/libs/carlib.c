...
bool verifyModel(char model[]) {
  ...
  for (size_t i = 0; i < strlen(model); ++i) {
    if ((model[i] - 'A' < 0 || model[i] - 'A' > 'Z' - 'A') &&
        (model[i] - 'a' < 0 || model[i] - 'a' > 'z' - 'a') && model[i] != ' ' &&
        model[i] != '-')
      return false;
  }
  return true;
...
bool verifyMileage(float m) { return (m >= 0); }

car createCar(char model[], const char *owner, float mileage) {
  bool verified = true;

  if (!verifyModel(model)) {
    logError("car model must be a string up to 16 characters long,"
...
void printCar(car c) {
  printf("Car<model: %s, owner: %s, mileage: %f>", c.model, c.owner, c.mileage);
}
