...
template <class T> Deque<T>::Deque() { sequence = new LinkedListSequence<T>(); }

template <class T> Deque<T>::Deque(T *items, int count) {
  sequence = new LinkedListSequence<T>(items, count);
}

template <class T> Deque<T>::Deque(const Sequence<T> *other_seq) {
...
template <class T> void Deque<T>::push_front(const T &item) {
  sequence->prepend(item);
}

template <class T> void Deque<T>::push_back(const T &item) {
...
template <class T> T Deque<T>::front() const {
  if (empty())
    throw kogan::EmptyContainerException();
...
template <class T> T Deque<T>::back() const {
  if (empty())
    throw kogan::EmptyContainerException();
...
template <class T> Deque<T> *Deque<T>::concat(const Deque<T> &other) const {
