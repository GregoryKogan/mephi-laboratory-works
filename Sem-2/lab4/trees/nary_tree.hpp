...
template <class T>
NaryTree<T>::NaryTree(T root_value)
    : NaryTree(root_value, std::numeric_limits<int>::max()) {
} // NOLINT(cppcoreguidelines-pro-type-member-init)

template <class T>
NaryTree<T>::NaryTree(T root_value, int max_children)
...
template <class T> NaryTree<T>::NaryTree(const NaryTree<T> &tree) {
  children = new LinkedListSequence<NaryTree<T> *>;
...
template <class T> NaryTree<T>::~NaryTree() {
  for (int i = 0; i < children->get_length(); ++i)
    delete children->get(i);
  delete children;
}

template <class T> size_t NaryTree<T>::size() const {
  size_t result = 1;
  for (int i = 0; i < children->get_length(); ++i)
...
template <class T> size_t NaryTree<T>::children_count() const {
  return children->get_length();
}

template <class T> size_t NaryTree<T>::max_children_count() const {
  return (size_t)max_children;
}

template <class T> size_t NaryTree<T>::height() const {
...
template <class T> T NaryTree<T>::get_data() const { return data; }

template <class T> void NaryTree<T>::set_data(T new_data) { data = new_data; }

template <class T> void NaryTree<T>::add_child(T child) {
...
template <class T> void NaryTree<T>::add_child(NaryTree<T> *child) {
  if (children->get_length() == max_children)
    throw IndexOutOfRangeException(max_children + 1, 0, max_children);
...
template <class T> NaryTree<T> *NaryTree<T>::get_child(int index) const {
  return children->get(index);
}

template <class T> void NaryTree<T>::remove_child(int index) {
  delete children->get(index);
  children->remove(index);
}

template <class T> Sequence<T> *NaryTree<T>::traverse() const {
...
template <class T> Sequence<T> *NaryTree<T>::traverse_root_left_right() const {
...
template <class T> Sequence<T> *NaryTree<T>::traverse_root_right_left() const {
  Sequence<T> *result = new LinkedListSequence<T>;
  Stack<const NaryTree<T> *> stack;
  stack.push(this);
...
template <class T> Sequence<T> *NaryTree<T>::traverse_left_right_root() const {
  Sequence<T> *result = new LinkedListSequence<T>;
...
template <class T> Sequence<T> *NaryTree<T>::traverse_right_left_root() const {
  Sequence<T> *result = new LinkedListSequence<T>;
...
template <class T> std::string NaryTree<T>::to_string() const {
...
template <class T> NaryTree<T> *NaryTree<T>::map(T (*func)(T)) const {
...
template <class T> NaryTree<T> *NaryTree<T>::where(bool (*func)(T)) const {
  if (!func(get_data()))
    return nullptr;
...
template <class T> int NaryTree<T>::count(T value) const {
  Sequence<T> *path = traverse();
  int result = 0;
...
template <class T> NaryTree<T> &NaryTree<T>::operator[](int index) {
