...
type_t *type_ctor(error *err, const char *name, void *zero, void *one,
                  ...
                  void (*random)(void *), char *(*to_string)(error *, void *)) {
...
type_t *type_copy(error *err, type_t *original) {
  return type_ctor(
      err, original->name, original->from_instance(err, original->zero),
      original->from_instance(err, original->one), original->from_instance,
      original->free_memory, original->add, original->sub, original->mul,
      original->random, original->to_string);
}
