...
TEST(constructor_copy, nary_tree_test_suite) {
  ...
  ASSERT(serializer.get_serialized_tree() ==
         deserializer.get_serialized_tree());
}
...
TEST(to_string, nary_tree_test_suite) {
  ...
  ASSERT(tree.to_string() == "[F, B, A, D, C, E, G, I, H]");
...
TEST(deserialize_invalid, nary_tree_test_suite) {
  ...
  try {
    kogan::NaryTreeSerializer<int> serializer(
        "2 F(B(A(), D(C()E(, )))G(I(H())))");
    ASSERT(false);
  } catch (kogan::InvalidArgumentException &e) {
    ASSERT(true);
...
TEST(map, nary_tree_test_suite) {
  ...
  kogan::NaryTree<int> *mapped_tree =
      deserializer.get_tree()->map([](int x) { return x * 2; });
...
TEST(where, nary_tree_test_suite) {
  ...
  kogan::NaryTree<int> *filtered_tree =
      deserializer.get_tree()->where([](int x) { return x < 6; });
...
kogan::TestSuite get_nary_tree_test_suite() { return nary_tree_test_suite; }
