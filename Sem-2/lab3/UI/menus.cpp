...
void get_subsequence_menu(kogan::BaseContainer<int> *container) {
  ...
  kogan::Sequence<int> *sub_seq =
      container->get_subsequence(start_index, end_index);
...
void concat_queue_menu(kogan::Queue<int> *queue) {
  log_blue("Concatenate");
  std::cout << std::endl;

  std::cout << "Other queue's length: ";
  int other_length;
  std::cin >> other_length;
