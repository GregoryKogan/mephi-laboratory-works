...
bool transpose_test(error *err) {
  ...
  bool result = true;
  if (t == NULL || m == NULL || matrix_get_width(t) != matrix_get_height(m) ||
      matrix_get_height(t) != matrix_get_width(m))
    result = false;

  int target[12] = {4, 5, 6, 5, 7, 9, 6, 9, 12, 7, 11, 15};
  if (result) {
    for (size_t i = 0; i < 12; ++i) {
      if (*((int *)matrix_get_value(err, t, i / 3, i % 3)) != target[i])
        result = false;
    }
  }
...
bool addition_test(error *err) {
  ...
  if (matrix_get_width(sum) != 4 || matrix_get_height(sum) != 3)
    result = false;

  int target[12] = {2, 4, 6, 8, 4, 7, 10, 13, 6, 10, 14, 18};
  if (result) {
    for (size_t i = 0; i < 12; ++i) {
      if (*((int *)matrix_get_value(err, sum, i / 4, i % 4)) != target[i])
        result = false;
    }
  }
...
bool addition_size_conflict_test(error *err) {
...
bool addition_type_conflict_test(error *err) {
...
bool addition_of_scalar_test(error *err) {
  ...
  float target[12] = {-2, -1, 0, 1, -1, 1, 3, 5, 0, 3, 6, 9};
  if (result) {
    for (size_t i = 0; i < 12; ++i) {
      if (fabsf(*((float *)matrix_get_value(err, res_m, i / 4, i % 4)) -
                target[i]) > 0.001)
        result = false;
    }
  }
...
bool multiplication_test(error *err) {
  ...
  int target[9] = {14, 20, 26, 20, 30, 40, 26, 40, 54};
  if (result) {
...
bool multiplication_by_scalar_test(error *err) {
  ...
  float target[12] = {3, 6, 9, 12, 6, 12, 18, 24, 9, 18, 27, 36};
  if (result) {
    for (size_t i = 0; i < 12; ++i) {
      if (fabsf(*((float *)matrix_get_value(err, res_m, i / 4, i % 4)) -
                target[i]) > 0.001)
...
bool linear_combination_test(error *err) {
  ...
  alphas[1] = &a2;
  matrix_t *result_m =
      matrix_add_linear_combination(err, m, 1, (const void **)alphas);
  free(alphas);
  ...
  float target[12] = {1, 2, 3, 4, 25, 50, 75, 100, 3, 6, 9, 12};
  if (result) {
    for (size_t i = 0; i < 12; ++i) {
      if (fabsf(*((float *)matrix_get_value(err, result_m, i / 4, i % 4)) -
                target[i]) > 0.001)
        result = false;
    }
  }
...
test_t **get_matrix_tests(error *err, size_t *tests_num) {
  *tests_num = 14;
  ...
  tests[0] = test_ctor(err, "matrix constructor dimensions",
                       constructor_dimensions_test);
  tests[1] = test_ctor(err, "matrix constructor initial values",
                       constructor_initial_values_test);
  tests[2] = test_ctor(err, "matrix constructor edge cases",
                       constructor_edge_cases_test);
  ...
  tests[6] = test_ctor(err, "matrix addition size conflict",
                       addition_size_conflict_test);
  tests[7] = test_ctor(err, "matrix addition type conflict",
                       addition_type_conflict_test);
  tests[8] =
      test_ctor(err, "matrix addition of scalar", addition_of_scalar_test);
  tests[9] = test_ctor(err, "matrix multiplication", multiplication_test);
  tests[10] = test_ctor(err, "matrix multiplication size conflict",
                        multiplication_size_conflict_test);
  tests[11] = test_ctor(err, "matrix multiplication type conflict",
                        multiplication_type_conflict_test);
  tests[12] = test_ctor(err, "matrix multiplication by scalar",
                        multiplication_by_scalar_test);
  tests[13] =
      test_ctor(err, "matrix linear combination", linear_combination_test);
