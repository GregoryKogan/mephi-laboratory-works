...
template <class T> DynamicArray<T>::DynamicArray(T *items, size_t count) {
  length = count;
  data = new T[length];
...
template <class T> size_t DynamicArray<T>::get_length() const { return length; }

template <class T> void DynamicArray<T>::set(int index, T item) {
  if (index < 0 || index >= length)
    throw IndexOutOfRangeException(index, 0, length - 1);
  data[index] = item;
}
