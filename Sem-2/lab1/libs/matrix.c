...
void matrix_dtor(matrix_t *self) {
  if (self == NULL)
    return;
  for (size_t i = 0; i < matrix_get_width(self) * matrix_get_height(self);
       ++i) {
    if (self->data[i] != NULL)
...
size_t matrix_get_width(matrix_t *self) { return self->width; }

size_t matrix_get_height(matrix_t *self) { return self->height; }
...
void matrix_set_value(error *err, matrix_t *self, size_t i, size_t j,
                      const void *value) {
  if (i >= matrix_get_height(self) || j >= matrix_get_width(self)) {
    error_raise(err, "matrix index out of range");
    return;
...
matrix_t *matrix_copy(error *err, matrix_t *m) {
  matrix_t *result = matrix_ctor(err, type_copy(err, m->type),
                                 matrix_get_height(m), matrix_get_width(m));
...
matrix_t *matrix_transpose(error *err, matrix_t *self) {
  matrix_t *result =
      matrix_ctor(err, type_copy(err, self->type), matrix_get_width(self),
  ...
  for (size_t j_ind = 0; j_ind < matrix_get_width(self); ++j_ind) {
    for (size_t i_ind = 0; i_ind < matrix_get_height(self); ++i_ind)
      matrix_set_value(err, result, j_ind, i_ind,
                       matrix_get_value(err, self, i_ind, j_ind));
  }
  return result;
}
...
matrix_t *matrix_add(error *err, matrix_t *a, matrix_t *b) {
  ...
  if (matrix_get_height(a) != matrix_get_height(b) ||
      matrix_get_width(a) != matrix_get_width(b)) {
    error_raise(err, "matrix sizes do not match");
    return NULL;
  }

  matrix_t *result = matrix_ctor(err, type_copy(err, a->type),
                                 matrix_get_height(a), matrix_get_width(a));
  ...
  for (size_t i = 0; i < matrix_get_height(a); ++i) {
    for (size_t j = 0; j < matrix_get_width(a); ++j) {
      ...
      a->type->add(res_ptr, matrix_get_value(err, a, i, j),
                   matrix_get_value(err, b, i, j));
...
matrix_t *matrix_mul(error *err, matrix_t *a, matrix_t *b) {
  ...
  matrix_t *result = matrix_ctor(err, type_copy(err, a->type),
                                 matrix_get_height(a), matrix_get_width(b));
  ...
  for (size_t a_row = 0; a_row < matrix_get_height(a); ++a_row) {
    for (size_t b_col = 0; b_col < matrix_get_width(b); ++b_col) {
      ...
      for (size_t iter = 0; iter < iters; ++iter) {
        a->type->mul(sum_buf, matrix_get_value(err, a, a_row, iter),
...
matrix_t *matrix_add_linear_combination(error *err, matrix_t *m,
                                        size_t row_index, const void **alphas) {
  matrix_t *result = matrix_copy(err, m);
...
char *matrix_to_string(error *err, matrix_t *self) {
  char *str = (char *)malloc(sizeof(char) * matrix_get_width(self) *
                                 matrix_get_height(self) * 10 +
                             128);
  ...
  snprintf(buff, 256, "<Matrix [%lu x %lu]> (type: %s)\n",
           matrix_get_height(self), matrix_get_width(self), self->type->name);
  ...
  for (size_t i = 0; i < matrix_get_height(self); ++i) {
    for (size_t j = 0; j < matrix_get_width(self); ++j) {
      char *value_str =
          self->type->to_string(err, matrix_get_value(err, self, i, j));
