...
template <class T> LinkedList<T>::LinkedList(T *items, size_t count) {
  length = count;
  root = new list_root;
...
template <class T> T LinkedList<T>::get_first() {
  if (length == 0)
    throw EmptyContainerException();
  return root->head->value;
}

template <class T> T LinkedList<T>::get_last() {
  if (length == 0)
    throw EmptyContainerException();
...
template <class T> size_t LinkedList<T>::get_length() { return length; }

template <class T> void LinkedList<T>::set(int index, T item) {
  if (index < 0 || index >= length)
    throw IndexOutOfRangeException(index, 0, length - 1);
...
template <class T> void LinkedList<T>::insert(int index, T item) {
  if (index < 0 || index > length)
